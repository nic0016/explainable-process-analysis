"""Training utilities for all models (XGBoost, ResNet, TCN, BiLSTM, BERT, GPT, TFT)."""
...
def get_resource_usage() -> Dict[str, float]:
    """Measure current resource usage (CPU, RAM, GPU)."""
    resources = {
    ...
    if torch.cuda.is_available():
        try:
            ...
            resources['gpu_utilization_percent'] = torch.cuda.utilization() if hasattr(torch.cuda, 'utilization') else 0
        except:
...
def log_csv_row(csv_path: str, row: Dict[str, float | int | str]) -> None:
...
def train_xgboost_per_epoch(
    ...
    """Train XGBoost model incrementally, logging metrics per epoch."""
    xgb = XGBRegressor(
        n_estimators=0,
        max_depth=8,
        learning_rate=0.1,
...
def build_cnn_loaders(encoded_sequences: np.ndarray, y: np.ndarray, batch_size: int = 64, random_state: int = 42) -> Tuple[DataLoader, DataLoader, torch.device]:
    """Build data loaders for CNN models (ResNet, TCN)."""
    E = encoded_sequences.astype(np.float32)
    N, K, M = E.shape
    E = np.transpose(E, (0, 2, 1))  # [N, K, M] -> [N, M, K]
...
def build_token_loaders(encoded_sequences: np.ndarray, y: np.ndarray, batch_size: int = 64, random_state: int = 42) -> Tuple[DataLoader, DataLoader, torch.device, int]:
    """Build data loaders for token-based models (BERT, GPT)."""
    ...
    val_loader = DataLoader(val_ds, batch_size=batch_size, shuffle=False)
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    vocab_size = M + 1  # +1 for padding id 0
    return train_loader, val_loader, device, vocab_size


def train_resnet(encoded_sequences: np.ndarray, y: np.ndarray, max_epochs: int, csv_path: str | None, dataset_name: str | None, in_channels: int) -> Dict[str, List[float]]:
    """Train ResNet model."""
...
def train_tcn(encoded_sequences: np.ndarray, y: np.ndarray, max_epochs: int, csv_path: str | None, dataset_name: str | None, in_channels: int) -> Dict[str, List[float]]:
    """Train TCN model."""
...
def train_bilstm(encoded_sequences: np.ndarray, y: np.ndarray, max_epochs: int, csv_path: str | None, dataset_name: str | None, input_size: int) -> Dict[str, List[float]]:
    """Train BiLSTM model."""
...
def train_bert(encoded_sequences: np.ndarray, y: np.ndarray, max_epochs: int, csv_path: str | None, dataset_name: str | None) -> Dict[str, List[float]]:
    """Train BERT model."""
    ...
    def to_loader(dl: DataLoader) -> DataLoader:
        xs, ys = [], []
        for xb, yb in dl:
            xs.append(xb)
            ys.append(yb)
    ...
    def runner(model: nn.Module, train_loader: DataLoader, val_loader: DataLoader, max_epochs: int, device: torch.device) -> Dict[str, List[float]]:
        criterion = nn.MSELoss()
        optimizer = optim.Adam(model.parameters(), lr=1e-3)
        r2_train, r2_val, mae_train, mae_val = [], [], [], []
        ...
        for epoch in range(1, max_epochs + 1):
            model.train()
            tr_preds, tr_targets = [], []
            for tokens, yb, mask in train_loader:
            ...
            va_preds, va_targets = [], []
            with torch.no_grad():
                for tokens, yb, mask in val_loader:
                    tokens = tokens.to(device)
                    yb = yb.to(device)
                    mask = mask.to(device)
                    pred = model(tokens, padding_mask=mask)
            ...
            if csv_path and dataset_name:
                log_csv_row(csv_path, {
                    'dataset': dataset_name, 'model': 'BERT', 'epoch': epoch,
                    'r2_train': r2_tr, 'r2_val': r2_va, 'mae_train': mae_tr, 'mae_val': mae_va
                })
        return {'r2_train': r2_train, 'r2_val': r2_val, 'mae_train': mae_train, 'mae_val': mae_val}

    return runner(model, train_loader2, val_loader2, max_epochs, device)


def train_gpt(encoded_sequences: np.ndarray, y: np.ndarray, max_epochs: int, csv_path: str | None, dataset_name: str | None) -> Dict[str, List[float]]:
    """Train GPT model."""
    ...
    def runner(model: nn.Module, train_loader: DataLoader, val_loader: DataLoader, max_epochs: int, device: torch.device) -> Dict[str, List[float]]:
        ...
        for epoch in range(1, max_epochs + 1):
            ...
            if csv_path and dataset_name:
                log_csv_row(csv_path, {
                    'dataset': dataset_name, 'model': 'GPT', 'epoch': epoch,
                    'r2_train': r2_tr, 'r2_val': r2_va, 'mae_train': mae_tr, 'mae_val': mae_va
                })
        return {'r2_train': r2_train, 'r2_val': r2_val, 'mae_train': mae_train, 'mae_val': mae_val}
...
def train_tft(encoded_sequences: np.ndarray, y: np.ndarray, max_epochs: int, csv_path: str | None, dataset_name: str | None, input_size: int) -> Dict[str, List[float]]:
    """Train TFT model."""
